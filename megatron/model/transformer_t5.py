...
class CoreAttention(MegatronModule):
    ...
    def __init__(self, layer_number,
        ...
        if self.layer_number == 1:
            self.relative_attention_bias = torch.nn.Embedding(self.relative_attention_num_buckets, self.num_attention_heads_per_partition)
    ...
    def compute_bias(self, query_length, key_length):
        ...
        relative_position_bucket = self._relative_position_bucket(
            ...
            bidirectional=(not self.layer_type == LayerType.decoder),
    ...
    def forward(self, query_layer, key_layer,
        ...
        # ===========================
        if not self.layer_number == 1: #has_relative_attention_bias:
            position_bias = torch.zeros(
                (1, self.num_attention_heads_per_partition, sq, sk), device=attention_scores.device, dtype=attention_scores.dtype
            )
        else:
            position_bias = self.compute_bias(sq, sk)
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def __init__(self, init_method, output_layer_init_method,
        ...
        if self.layer_type == LayerType.decoder:
            ...
            #     no_persist_layer_norm=args.no_persist_layer_norm,
            #     sequence_parallel=args.sequence_parallel)
            self.post_inter_attention_layernorm = T5LayerNorm(
                args.hidden_size,
                eps=args.layernorm_epsilon,
            )
